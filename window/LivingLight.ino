...
void livingLight() {
  ...
  for(uint8_t i=0; i<NUM_LEDS; i++) {
    leds[i] = ColorFromPalette(redglow, colourIndex[i], 255, LINEARBLEND);
  }
}
...
void randomiseSpeed() {
  ...
  for(uint8_t i=1; i<NUM_LEDS-1; i++) {
    ...
    if( deviation > MAX_GRADIANT ) {
      colourSpeed[i] = constrain( colourSpeed[i]+random(0,3), -MAX_SPEED, MAX_SPEED);
    } else if( deviation < -MAX_GRADIANT ) {
      colourSpeed[i] = constrain( colourSpeed[i]+random(-2,1), -MAX_SPEED, MAX_SPEED);
    } else if(random8()<64) {
      colourSpeed[i] = constrain( colourSpeed[i] + random(-1,2), -MAX_SPEED, MAX_SPEED);
    }
  }
}
